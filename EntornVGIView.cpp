...
void CEntornVGIView::OnPaint()
{
	...
	*/
	...
	case ORTO:
		...
		glDisable(GL_SCISSOR_TEST);  // Important per no retallar les línies
		...
		glPushMatrix();
		glLoadIdentity();
		glOrtho(0, w, 0, h, -1, 1);  // Coordenades en píxels
		...
		glBegin(GL_LINES);
		// Línia vertical (centre pantalla)
		glVertex2f(w / 2.0f, 0.0f);
		glVertex2f(w / 2.0f, (float)h);

		// Línia horitzontal (centre pantalla)
