...
void camio(GLuint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[5])
{
	...
	const float remolque_height = 1.0f;
	...
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0f, -(remolque_length / 2 - cabina_side / 2), centrado_z));
	...
	ModelMatrix = glm::translate(MatriuTG, vec3(0.0f, (remolque_length / 2 - cabina_side / 2), rueda_r_ext + cabina_side / 2));
	ModelMatrix = glm::scale(ModelMatrix, vec3(cabina_side, cabina_side, cabina_side));
	...
	col_object = { 0.7, 0.7, 0.7, 10.0 };
	SeleccionaColorMaterial(sh_programID, col_object, sw_mat);

	auto rueda = [&](float x, float y)
		{
			ModelMatrix = glm::translate(MatriuTG, vec3(x, y, rueda_r_ext));
